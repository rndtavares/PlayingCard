//
//  PlaingCardView.swift
//  PlayingCard
//
//  Created by Ronaldo Tavares da Silva on 11/08/2020.
...
class PlayingCardView: UIView {
    ...
    private lazy var upperRightCornerLabel = createCornerLabel()
    ...
    override func draw(_ rect: CGRect) {
        let roundedRect = UIBezierPath(roundedRect: bounds, cornerRadius: corderRadius)
        roundedRect.addClip()
...
//            if let faceCardImage = UIImage(named: "JJ♠", in: Bundle(for: self.classForCoder), compatibleWith: traitCollection) {

                faceCardImage.draw(in: bounds.zoom(by: SizeRatio.faceCardImageToBoundsSize))
            } else {
                drawPips()
            }
        } else {
...
extension PlayingCardView {
    ...
    private var corderRadius: CGFloat {
        return bounds.size.height * SizeRatio.cornerRadiusToBoundsHeight
    }
    private var cornerOffset: CGFloat {
        return corderRadius * SizeRatio.cornerOffsetToCornerRadius
    }
